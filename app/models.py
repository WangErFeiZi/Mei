...
class User(UserMixin, db.Model):
    ...
    def generate_confirmation_token(self, expiration=3600, email=None, key=None):
        s = Serializer(current_app.config['SECRET_KEY'], expiration)
        return s.dumps({'user_id': self.id, 'email': email, 'key': key}).decode('utf-8')
...
class Post(db.Model):
    ...
    def from_json(json_post):
        ...
        if body is None or body == '':
            raise ValidationError('post does not have a body 文章没有内容.')
...
class Comment(db.Model):
    ...
    def from_json(json_comment):
        ...
        if body is None or body == '':
            raise ValidationError('comment does not have a body 评论没有内容.')
